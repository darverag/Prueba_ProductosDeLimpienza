...
public class CrearProducto extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (nombre_producto ==null || nombre_producto.equals("") ||  precio_producto.equals("") || precio_producto == null ||  categoriaId == null
				|| categoriaId.equals("") || descripcion_producto.equals("") || descripcion_producto ==null) {
			request.setAttribute("msgError", "Error en la creación,  información faltante");
		
		}else {
			Categoria categoria = new Categoria();
			categoria.setId_categoria(Integer.parseInt(categoriaId));

			Producto producto = new Producto(nombre_producto,Integer.parseInt(precio_producto), descripcion_producto, categoria);
			
		//se llama al metodo en el facadeProducto		
		int resultado = facadeProducto.crearProducto(producto);
		if(resultado == 1) {
			request.setAttribute("msgExito", "Se creó correctamente el nuevo producto. Para eliminar o modificar ir a Listar Productos");			
		}else {
